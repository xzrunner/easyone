...
void WxStageCanvas::OnDrawGUI() const
{
	...
	y -= h;
	if (egui::radio_button(uid++, "POLYLINE", shape_type == rttr::type::get<gs::Polyline>().get_id(), x, y, *ctx, dirty)) {
		m_edit_shape_op->ChangeEditState(rttr::type::get<gs::Polyline>().get_id());
	}
	y -= h;
