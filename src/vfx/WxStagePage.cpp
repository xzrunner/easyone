...
const std::string WxStagePage::PAGE_TYPE = "vfx_graph";
...
//    , m_eval(std::make_shared<vfxlab::Evaluator>())
{
...
//    vfxlab::Blackboard::Instance()->SetEval(m_eval);
...
{
	auto tree = static_cast<vfxlab::WxGraphPage*>(m_graph_page)->GetSceneTree();
...
void WxStagePage::OnPageInit()
{
    ...
    auto prev_canvas = std::static_pointer_cast<vfxlab::WxPreviewCanvas>(GetImpl().GetCanvas());
...
{
	auto node = static_cast<vfxlab::WxGraphPage*>(m_graph_page)->GetSceneTree()->GetCurrNode();
	                             ...
	                             rapidjson::MemoryPoolAllocator<>& alloc) const
{
    ...
    bp::Serializer<vfxgraph::Variant>::StoreToJson(node, dir, val, alloc);
...
void WxStagePage::LoadFromFileExt(const std::string& filepath)
{
    ...
    {
        static_cast<vfxlab::WxGraphPage*>(m_graph_page)->SetFilepath(filepath);

        rapidjson::Document doc;
        js::RapidJsonHelper::ReadFromFile(filepath.c_str(), doc);

        auto dir = boost::filesystem::path(filepath).parent_path().string();
        bp::Serializer<vfxgraph::Variant>::LoadFromJson(m_dev, *m_graph_page, m_graph_obj, doc, dir);
...
{
    auto panel = new vfxlab::WxGraphPage(m_dev, parent, m_graph_obj, m_sub_mgr);
    auto canvas = std::make_shared<vfxlab::WxGraphCanvas>(
    ...
    auto& nodes = vfxlab::VfxLab::Instance()->GetAllNodes();
    ...
    if (page_type >= 0)
    {
        ...
        if (page_type == PAGE_VFX_GRAPH)
        {
            ...
            if ((*obj)->HasUniqueComp<bp::CompNode>())
            {
                ...
                {
                    auto sg_stage_page = static_cast<vfx::WxStagePage*>(stage_page);
